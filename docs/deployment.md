...
| [[roselight]] | https://theroselight.org (prod) · `www`→apex 301 · lab https://lab.theroselight.org · previews `<PR#>.theroselight.xyz` | web `ckgxbv0nlhgh0v52jxh2ul07` · worker `m6zinqoryw8iivs2vcptxusa` | 🟢 **live on PROD** (cut over from DigitalOcean 2026-06-21; prod DB migrated into shared PG `public` schema, DO retired). Two apps: web (**Nixpacks**, start cmd `npm run db:migrate:deploy && npm run start`) + node-cron worker (`Dockerfile.worker`, no public route — RSS sync + weekly digest). Worker is the sole RSS ingester; if stories go stale, the worker is down. **Monitoring:** worker heartbeat → healthchecks.io (`HEALTHCHECK_URL` env, ~15m period/grace, emails on miss) + Coolify container/deploy email alerts (Sentinel on). |
